...
void OpenGLESPage::OnPageLoaded(Platform::Object^ sender, Windows::UI::Xaml::RoutedEventArgs^ e)
{
    ...
    CreateRenderSurface();
	mMoaiHost.reset(new MoaiHost());
	DisplayInformation^ currentDisplayInformation = DisplayInformation::GetForCurrentView();
	mOpenGLES->MakeCurrent(mRenderSurface);
	mMoaiHost->Init(swapChainPanel->RenderSize.Width, swapChainPanel->RenderSize.Height, (int)currentDisplayInformation->LogicalDpi);
    StartRenderLoop();
}
...
void OpenGLESPage::RecoverFromLostDevice()
{
	...
	mMoaiHost->RedetectGraphicsContext();
    StartRenderLoop();
}
...
void OpenGLESPage::StartRenderLoop()
{
    ...
    auto workItemHandler = ref new Windows::System::Threading::WorkItemHandler([this](Windows::Foundation::IAsyncAction ^ action)
    {
        ...
        mOpenGLES->MakeCurrent(mRenderSurface);
		uint64 lastFrame = GetTickCount64();
       ...
       //     GetSwapChainPanelSize(&panelWidth, &panelHeight);
			...
			uint64 frameDelta = (uint64)(AKUGetSimStep() * 1000.0);
			uint64 currentFrame = GetTickCount64();
			uint64 delta = currentFrame - lastFrame;
			if (delta < frameDelta) {
				Sleep(frameDelta - delta);
			}
