...
SDL_Window* MOAIDisplaySDL::m_window = NULL;

MOAIDisplaySDL::MOAIDisplaySDL(){
	RTTI_BEGIN
 ...
 */
void MOAIDisplaySDL::RegisterLuaClass(MOAILuaState& state) {
	luaL_Reg regTable[] = {
 ...
 * @param [in,out]	state	MOAI's current Lua state.
 */
void MOAIDisplaySDL::RegisterLuaFuncs(MOAILuaState& state) {
	luaL_Reg regTable[] = {
		{ NULL, NULL}
	};
	luaL_register(state, 0, regTable);
...
int MOAIDisplaySDL::DoGetScreenList( lua_State* L )
{
	MOAI_LUA_SETUP(MOAIDisplaySDL, "U")
 ...
 */
int MOAIDisplaySDL::DoGetResolutionInfo( lua_State* L )
{
 ...
 * @return	nil
 */
int MOAIDisplaySDL::DoSetResolution( lua_State* L )
{
 ...
 */
void MOAIDisplaySDL::SetWindowHandle(SDL_Window* p_window)
{
	MOAIDisplaySDL::m_window = p_window;
}
